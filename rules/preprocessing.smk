...
rule NCBI_download:
    ...
    """
    input:
        results+"/intermediate/ncbi/{sample}"
    output:
        fwd = temp(results+"/intermediate/preprocess/{sample}_{unit}_R1"+preprocess_suffices["trimming"]+".fastq.gz"),
        rev = temp(results+"/intermediate/preprocess/{sample}_{unit}_R2"+preprocess_suffices["trimming"]+".fastq.gz")
    log:
        results+"/intermediate/preprocess/{sample}_{unit}"+".NCBI.log"        
    params:
        location = results+"/intermediate/preprocess/",
        R1_unzipped = temp(results+"/intermediate/preprocess/{sample}_1.fastq"),
        R2_unzipped = temp(results+"/intermediate/preprocess/{sample}_2.fastq")
    conda:
        "../envs/NCBI_tools.yml"
    ...
    shell:
        """
        #prefetch {wildcards.sample} --max-size 70GB -O {params.location}  > {log} 2>&1
        fasterq-dump  -e {threads} -S -O {params.location} {wildcards.sample}  > {log} 2>&1
        gzip {params.R1_unzipped} -c > {output.fwd} 
        gzip {params.R2_unzipped} -c > {output.rev}
        rm {params.R1_unzipped} {params.R2_unzipped} 
        """
##bbduk###
from scripts.common import get_bbduk_string
rule bbduk_pe:
    input:
    ...
    output:
        R1 = temp(results+"/intermediate/preprocess/{sample}_{unit}_R1"+preprocess_suffices["trimming"]+".bbduk.fastq.gz"), 
        R2 = temp(results+"/intermediate/preprocess/{sample}_{unit}_R2"+preprocess_suffices["trimming"]+".bbduk.fastq.gz"),
    log:
        R1log=results+"/intermediate/preprocess/{sample}_{unit}_R1"+preprocess_suffices["trimming"]+".bbduk.log",
        R2log=results+"/intermediate/preprocess/{sample}_{unit}_R2"+preprocess_suffices["trimming"]+".bbduk.log"
    params:
        bbduk_string = get_bbduk_string(config)
    threads: 30
    conda:
        "../envs/bbtools.yml"
    group: "ncbi_cent_diamond"
    shell:
        ...
        bbduk.sh \
            ...
            {params.bbduk_string}  -a {log.R1log} 
        sed \
...
rule trimmomatic_pe:
    input:
        ...
        R2=results+"/intermediate/preprocess/{sample}_{unit}_R2"+preprocess_suffices["trimming"]+".fastq.gz"
    output:
        R1P=temp(results+"/intermediate/preprocess/{sample}_{unit}_R1"+preprocess_suffices["trimming"]+".trimmomatic.fastq.gz"),
        R1U=temp(results+"/intermediate/preprocess/{sample}_{unit}_R1"+preprocess_suffices["trimming"]+".trimmomatic.U.fastq.gz"),
        R2P=temp(results+"/intermediate/preprocess/{sample}_{unit}_R2"+preprocess_suffices["trimming"]+".trimmomatic.fastq.gz"),
        R2U=temp(results+"/intermediate/preprocess/{sample}_{unit}_R2"+preprocess_suffices["trimming"]+".trimmomatic.U.fastq.gz"),
    log:
