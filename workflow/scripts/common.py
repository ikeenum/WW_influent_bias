...
def prepost_string(config):
    ...
    # Trimming
    if config["preprocessing"]["bbduk"]:
        PREPROCESS += ".bbduk"
        preprocess_suffices["phixfilt"] = preprocess_suffices[
                                              "trimming"] + ".bbduk"
    elif config["preprocessing"]["trimmomatic"]:
...
def preprocessing_input(config):
    if config["run_preprocessing"] or config["preprocessing"]["fastqc"] or config["preprocessing"]["ncbi"] :
    ...
    for sample in samples.keys():
        for unit in samples[sample].keys():
            if is_pe(samples[sample][unit]):
                ...
                pairs = ["se"]
                seq_type = "se"
            files += get_ncbi_files (sample, config)
...
def get_ncbi_files(sample, config):
    """Get all fastqc output"""
    if config["preprocessing"]["ncbi"]:
        files = expand(config["paths"]["results"]+"/intermediate/ncbi/{sample}",
                       sample=sample)
        return files
    return []
...
def get_trim_logs(sample, unit, pairs, config, d):
    ...
    config["preprocessing"]["cutadapt"] and not config["preprocessing"]["bbduk"]:
        return []
    if config["preprocessing"]["bbduk"]:
        trimmer = "bbduk"
    elif config["preprocessing"]["trimmomatic"]:
...
def get_bbduk_string(config):
    """
    Generates trimsetting string for BBduk
    :param seq_type: PE or SE depending on sequencing type
    :return: string
    """
    java_mem= config["bbduk"]["java_mem"]
    qtrim = config["bbduk"]["qtrim"]
    trimq = config["bbduk"]["trimq"]
    minlen = config["bbduk"]["minlen"]
    maxns = config["bbduk"]["maxns"]
    riebt = config["bbduk"]["rieb"]
    additional_params = config["bbduk"]["additional_params"]
    trimsettings = "-Xmx{} qtrim={} trimq={} minlen={} maxns={} rieb={} {}".format(java_mem, qtrim, trimq, minlen, maxns, riebt, additional_params)
    return trimsettings
...
def diamond_annotation_input(config):
    """
    ...
    f = []
    results = config["paths"]["results"]
    if config["diamond_annotation_database"]["ARGS"]:
        f.append(f"{results}/diamond/card_results.tsv")
    if config["diamond_annotation_database"]["Virulence"]:
        f.append(f"{results}/diamond/Virdb_results.tsv")
    if config["diamond_annotation_database"]["MGES"]:
        f.append(f"{results}/diamond/mobileOG_results.tsv")
    return f


def get_diamond_annotation_string(config):
    """
    Generates alignment parameters for diamond processing
    :return: string
    """
    query_cover = config["diamond_annotation"]["query_cover"]
    identity = config["diamond_annotation"]["identity"]
    mlen = config["diamond_annotation"]["mlen"]
    evalue = config["diamond_annotation"]["evalue"]
    gene_coverage = config["diamond_annotation"]["gene_coverage"]
    bowtie_16S_identity = config["diamond_annotation"]["bowtie_16S"]
    executables = config["diamond_annotation"]["executables"]
    settings = "--query_cover {} --diamond_identity {} --diamond_mlen {} --diamond_evalue {} --gene_coverage {} --bowtie_16s_identity {} --path_to_executables {}".format(query_cover, identity, mlen, evalue, gene_coverage, bowtie_16S_identity, executables)
    return settings 
